...
from parser.predict_setV3 import PREDICT_SET
...
# ---------------- expected tokens for errors ----------------
    ...
    def parse_wall_init(self):
        ...
        # Track delimiters inside wall expressions so error filtering can pick

        # the correct top-closer (e.g., only ')' when we're inside (...)).
    ...
    def parse_wall_rhs(self):
        ...
        if self.in_predict(PREDICT_SET['<wall_rhs>']):  # prod 147
        ...
        elif self.in_predict(PREDICT_SET['<wall_rhs_1>']):  # prod 148
        ...
        elif self.in_predict(PREDICT_SET['<wall_rhs_2>']):  # prod 149
        ...
        elif self.in_predict(PREDICT_SET['<wall_rhs_3>']):  # prod 150
    ...
    def parse_init_value(self):
        ...
        # Enable delimiter tracking for expression parsing.
        # This prevents statement-level parentheses (e.g., the '(' after 'for') from
        # polluting the delimiter stack and forcing expected ')' everywhere.
        self.track_delims += 1
        try:
            ...
            if self.track_delims <= 0:
                self.track_delims = 0
                # Avoid leaking delimiter context across separate expressions.
                self.delim_stack.clear()

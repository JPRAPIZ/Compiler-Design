...
from parser.predict_setV4 import PREDICT_SET
...
class Parser:
    ...
    def _filter_expected_by_delims(self, expected: list[str]) -> list[str]:
        # Filtering removed: always show the grammar's full expected set.
        return expected
...
# ---------------- expected tokens for errors ----------------
    ...
    def _dedupe_and_sort(self, items: list[str]) -> list[str]:
        # Stable, readable list for error messages.
        uniq: list[str] = []
        seen: set[str] = set()
        for x in items:
            if x not in seen:
        ...
        # Sort by category then alphabetically so output doesn't jump around.
        keywords: list[str] = []
        symbols: list[str] = []
        others: list[str] = []
        ...
        symbols.sort()
        others.sort()
        return keywords + symbols + others

    def syntax_error(self, nt: str):
        exp = self._dedupe_and_sort(self.expected_for(nt))
        got = f"{self.current_type} {self.current_lexeme!r}"
        self._add_error(f"Unexpected token {got}; Expected one of: {', '.join(exp)}")
